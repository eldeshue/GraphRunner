...
void static set_vp_func_instance_with_volk(VpVulkanFunctions& functions) {
...
namespace Util {
    ...
    /**
     * @brief calculate aligned size of resource.
     *
     ...
     * 
     * 예를 들면, 
     * buffer의 flush나 invalidate는 nonCoherentAtomSize
     * UBO의 경우 limit의 minUniformBufferOffsetAlignment
     * SSBO의 경우 limit의 minStorageBufferOffsetAlignment
     * staging buffer의 경우 optimalBufferCopyOffsetAlignment
    ...
    calculate_aligned_value(VkDeviceSize target_value, VkDeviceSize alignment) {
        return (target_value + alignment - 1) & ~(alignment - 1);
    }
